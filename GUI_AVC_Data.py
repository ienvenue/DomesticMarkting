...
月度线下干衣机 = ['销量', '销额', '单价', '月度', '品牌', '机型', '品类', '变频', '产品类型', '干衣方式', '品牌类型', '容量', '上市月度', '上市周度',
           '子品牌', '大区', '地市', '美的渠道',  '省份', '洗衣机美的片区', '洗衣机美的中心', '洗衣机美的中心2', '县市级别2', '价格段',
...
月度线下洗衣机 = ['销量', '销额', '单价', '月度', '品牌', '机型', '品类', '壁挂洗衣机', '变频', '产品类型', '产品系列', '尺寸', '电机类型', '耗电量',
           ...
           '智能', '转速', '子品牌', '大区', '地市', '美的渠道',  '省份', '洗衣机美的片区', '洗衣机美的中心', '洗衣机美的中心2', '县市级别2',
...
def avc_month_csv2db(file_path, table_name, type_name, cols):
    try:
...
def avc_week_csv2db(file_path, table_name, type_name, cols):
    try:
        df = pd.read_csv(file_path, usecols=cols, encoding="gbk", engine='python')
        df['年'] = df['周度'].map(lambda x: x.split('W')[0])
        df['周'] = df['周度'].map(lambda x: x.split('W')[1])
        df.to_sql(name=table_name, con=engine, if_exists=type_name, index=False)
        messagebox.showerror('提示', '导入成功')
