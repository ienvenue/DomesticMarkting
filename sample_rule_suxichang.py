...
sql_sampled = '''  select distinct c.area as 区域,c.center as 中心, a.门店编码, case when a.门店等级 in ('B级', 'A级', 'A+级') then 1
                ...
                where c.center = '苏锡常' and a.二级分类 in('TOP', '国美常规店', '苏宁常规店', '五星常规店')
                and a.经营状态 = '经营中'
...
replace_dic = {'BVL1D100NET': '国米系列', 'BVL1G100NET': '国米系列', 'BVL1D100EY': '小骑士系列', 'BVL1D80EY': '小骑士系列',
               ...
               'TBJ110-8188WUADCLT': '8088一桶洗系列', 'TD100NM06Y': 'TD100', 'TG100NM06Y': 'TD100'
               }
...
if __name__ == '__main__':
    ...
    print('正在导出' + date + '-苏锡常出样结果.xlsx')
    df_sampled.to_excel('E:/Share/' + date + r'-苏锡常出样结果.xlsx', sheet_name='出样结果', index=False)
    print('导出' + date + '-苏锡常出样结果.xlsx 完成')
    df_unsampled = df_sampled.drop('还需出样', 1).join(df_sampled['还需出样'].str.split(',', expand=True).stack(). \
        ...
        drop('已出型号', 1)
    print('正在导出' + date + '-苏锡常出样明细.xlsx')
    df_unsampled.to_excel('E:/Share/' + date + r'-苏锡常出样明细.xlsx', sheet_name='出样明细', index=False)
    print('导出' + date + '-苏锡常出样明细.xlsx 完成')
