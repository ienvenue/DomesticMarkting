...
sql_sampled = '''  select distinct c.area as 区域,c.center as 中心, a.门店编码, case when a.门店等级 in ('B级', 'A级', 'A+级') then 1
                ...
                where c.center = '武汉' and a.二级分类 in('TOP', '国美常规店', '苏宁常规店', '五星常规店')
                and a.经营状态 = '经营中'
...
replace_dic = {'BVL1D100NET': '国米系列', 'BVL1G100NET': '国米系列', 'BVL1D100EY': '小骑士系列', 'BVL1D80EY': '小骑士系列',
               ...
               'MG100T1WDQC+MH100-H1W': 'MG100+MH100', 'MD100-1455WDY': '1455N07系列', 'MG100-1455WDY': '1455N07系列',
               'MD100PD3QCT': 'PD3直驱系列', 'MG100PD3QCT': 'PD3直驱系列', 'MD100N07Y': '1455N07系列', 'MG100N07Y': '1455N07系列',
               ...
               'MBS90PT2WADT': '双驱波轮P系列', 'MBS100T2WADY': '双驱波轮全流通系列', 'MBS90T2WADY': '双驱波轮全流通系列',
                'TD100NM06Y': 'TD100', 'TG100NM06Y': 'TD100', 'TD100-14366WMUDT': 'TD100', 'TG100-14366WMUDT': 'TD100'
               }
...
if __name__ == '__main__':
    ...
    print('正在导出' + date + '-武汉出样结果.xlsx')
    df_sampled.to_excel('E:/Share/' + date + r'-武汉出样结果.xlsx', sheet_name='出样结果', index=False)
    print('导出' + date + '-武汉出样结果.xlsx 完成')
    df_unsampled = df_sampled.drop('还需出样', 1).join(df_sampled['还需出样'].str.split(',', expand=True).stack(). \
        ...
        drop('已出型号', 1)
    print('正在导出' + date + '-武汉出样明细.xlsx')
    df_unsampled.to_excel('E:/Share/' + date + r'-武汉出样明细.xlsx', sheet_name='出样明细', index=False)
    print('导出' + date + '-武汉出样明细.xlsx 完成')
